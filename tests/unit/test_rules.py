...
class TestInspectMakefile:
    ...
    def test_complete_parse_returns_report(
        ...
        assert facts.report["schema_version"] == 1
...
class TestBuildEnvelope:
    ...
    def test_empty_checkout_yields_inapplicable_envelope(
        ...
        assert applicability["root_cargo_toml"] is False
        assert applicability["root_makefile"] is False
    ...
    def test_full_checkout_yields_facts(
        ...
        cmd_mox.verify()
        applicability = typ.cast("dict[str, object]", envelope["applicability"])
        assert applicability["root_cargo_toml"] is True
        assert applicability["root_makefile"] is True
        cargo = typ.cast("dict[str, object]", envelope["cargo"])
        parsed = typ.cast("dict[str, object]", cargo["parsed"])
        assert parsed["package"] == {"name": "fixture", "version": "0.1.0"}
...
class TestRunRule:
    ...
    def test_failures_map_to_findings(
        ...
        assert result.findings[1].line == 3
...
class TestRendering:
    ...
    def test_table_lists_findings(self, result: RuleRunResult) -> None:
        ...
        assert "QG-001" in table
        assert "Makefile:1" in table
        assert "noncompliant" in table

    def test_json_round_trips(self, result: RuleRunResult) -> None:
        """Json round trips."""
        document = json.loads(render_json(result))
        assert document["rule_package"] == "rust-makefile-baseline"
        assert document["verdict"] == "noncompliant"
        assert document["findings"][0]["rule_id"] == "QG-001"

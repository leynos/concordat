...
- [x] (2026-07-19 16:20Z) Milestone E: `scripts/parabellum_sweep.py` with
  eight red-then-green tests, `docs/parabellum/estate.yaml` (52 repositories),
  and the append-only ledger. Canary over wireframe, netsuke, weaver, statelet,
  and fingermouse: all five `noncompliant` with exactly one finding each —
  QG-001 on `WHITAKER ?=` — matching a human reading (the Whitaker rollout
  recipe itself standardized `?=`); line citations spot-checked against
  wireframe `Makefile:13`.
- [x] (2026-07-19 17:30Z) Milestone F: full estate swept in four batches
  (52 repositories, ledger committed per batch);
  `docs/parabellum/baseline-report.md` generated by the new
  `parabellum_sweep report` subcommand; QG-001 added to the design document's
  §3.1 check catalogue; roadmap item 1.2 progress recorded; users' guide gained
  "Auditing a checkout against a lint rule package". Pre-rebaseline v0.1.0
  baseline: 44 noncompliant, 8 indeterminate, 0 compliant. Those figures
...
- Observation: the canary shows the estate's reference pattern itself
  violates QG-001 — `WHITAKER ?= whitaker` is what the Whitaker rollout recipe
  installed everywhere, so near-uniform `noncompliant` verdicts on that single
  finding are the expected estate baseline, not a policy bug. Evidence: all
  five canary repositories produced exactly one QG-001 `?=` finding; wireframe
  `Makefile:13` verified by hand. Impact: the eventual remediation wave will be
  estate-wide and mechanical (replace `?=` with `:=` or delegate to a canonical
  runner); the doctrine question of whether `?=` should remain permitted
...
- Decision: pin `makeutil` at commit
  ...
  reproducible pin; this SHA is recorded in every ledger record, and `--locked`
  pins dependency resolution to the `Cargo.lock` committed at that commit
  rather than re-solving it at install time. Date/Author: 2026-07-19, Fable
  (Milestone A).
...
The slice is complete and the campaign has its baseline. Against the original
purpose: `concordat artefact rule run` exists and answers both questions with
...
- `WHITAKER ?= whitaker` is not a finding. Policy v0.2.0 sanctions it as the
  estate convention — a local override is permitted because CI installs the
  real binary — so no remediation wave is owed for it.
- The remaining QG-001 findings are genuine gate gaps rather than the `?=`
  convention: actix-v2a and lille never invoke Whitaker at all, and
  repovec-appliance soft-skips the gate with `command -v`. whitaker and
  wildside reach the gate only beyond one prerequisite hop, which the rule
  reports as indeterminate rather than compliant.
- axinite and wildside lack a root `build` target (FP-003).
- Six repositories are mostly-Python/actions with Rust in subdirectories
  (AP-001): prosidy-darn, agent-template-rust, msgspec-crockford, cuprum,
  shared-actions, femtologging. Doctrine work: the eventual `.concordat`
  manifest must express nested-Cargo layouts before these can be governed.
- pg-embed-setup-unpriv's Makefile only parses with error recovery — a
  candidate for the makeutil corpus and possibly an upstream parser gap.
- whitaker itself fails the one-hop delegation proof; either the rule's
  hop bound needs a doctrine decision or whitaker's Makefile should delegate
  directly.

Lessons: the rule parameters pattern (`default` + scoped `data.parameters`)
avoids OPA recursion; conftest's default `main` namespace is a silent-pass
trap; fixture envelopes must be generated with relative paths to stay
machine-independent; the canary-then-batch cadence caught nothing surprising
because the fixtures already encoded the estate's real patterns — which is the
point.
...
- `concordat/rules/makefile_facts.py` —
  ...
  timeout, validates `schema_version == 1`, maps exit 1 to a recovered-status
  facts object and exit 2 to `ConcordatError`.
...
directory and the temporary run-log directory are mode `0700`; `mkdir -p` alone
does not guarantee this when the base directory already exists, hence the
explicit `chmod`. Run this once per session, and the `$RUN_LOG_DIR` used by the
examples below refers to it:
...
{"schema_version": 1, "repository": "leynos/wireframe",
 ...
 "rule_package": "rust-makefile-baseline", "rule_version": "0.2.0",
...
2026-07-19 (later): plan approved and executed the same day. All six milestones
landed; Progress, Surprises & discoveries, Decision log, and Outcomes &
retrospective updated throughout; status moved to COMPLETE. Deviations from the
draft are recorded in the living sections: a kept fixture generator, two
additive CmdMox harness changes, an `OperationalRuleError` subclass for exit 2,
a `report` subcommand on the sweep script, and local conftest 0.68.2 for
development against the CI pin. Remaining work is listed under follow-ups in
the retrospective.

...
- **The publisher serializes its baseline writes.** Two pushes to `main` in
  ...
  resolve the same for every push to `main`, because runs in different groups
  do not wait for each other. It may interpolate only `github.workflow`,
  `github.ref`, `github.ref_name` and `github.repository`; a group built on
  `github.run_id` or `github.sha` gives each run a group of its own.
